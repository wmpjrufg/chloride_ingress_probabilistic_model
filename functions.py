...
def generate_cross_section(img_w_mm: int, img_h_mm: int, dataset_csv: str = "dataset_contours_aggregate_by_patch.csv", 
                           dataset_json: str = "dataset_contours_aggregate_by_patch.json", 
                           output_json: str = "output_contour.json", 
                           output_img: str = "output_contour.png"):

    # Load dataset contours
    df = sort_contours_using_uniform_pdf_and_group(dataset_csv,dataset_json,350)
    ...
    for m, row in df.iterrows():
        ...
        centroids = sc.stats.qmc.scale(
            sampler.random(n=1),
            [0, 0],
            [img_w_px, img_h_px]
        ).squeeze()

        val_noise = 1 if m < 1 else 2
        cx = noise_point([centroids[0]], value_noise=val_noise)[0]
        cy = noise_point([centroids[1]], value_noise=val_noise)[0]
        angle = float(np.random.uniform(0, 360))

        x_new, y_new = trans_rota_polygon(
            row['x coordinate in 0,0'],
            row['y coordinate in 0,0'],
            cx, cy, angle=angle
        )
    ...
    plt.savefig(output_img, dpi=300, bbox_inches='tight', pad_inches=0)
    plt.show()